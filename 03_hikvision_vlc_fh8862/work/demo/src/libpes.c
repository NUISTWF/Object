...
struct vlcview_info
{
    ...
    struct sockaddr_in target_addr1;
};
...
static FILE *ps_file2 = NULL;

typedef struct threadArgNode {
    int fd;
    int ch;
} threadArg;
/*
...
void *thread_function(void *arg)
{
    ...
    while (!g_stop_request)
    {
        ...
        {
            pkt = ts_get(&g_vlc_channel_info[i]); // 是否有足够的数据包可供处理
            if (pkt)
            {
                if(stream_tag == 1){
                    _VLC_UNLOCK();
                    //ret = sendto(threadarg->fd, pkt->buf,  pkt->len, 0, (struct sockaddr *)&g_vlc_channel_info[i].target_addr, sizeof(struct sockaddr_in));
                    //if (ret < 0)
                    //{
                    ...
                    #if 1
                    if (i == 0) {
                        if ((stream_key1 == 1) && (status == 0)) {
                            gettimeofday(&tv1, NULL);
                            status = 1;
                        } 
                        if (status == 1) {
                            gettimeofday(&tv2, NULL);
                            long long diff = tv2.tv_sec - tv1.tv_sec;
                            //long long micro_diff = (tv2.tv_usec - tv1.tv_usec) / 1000000;
                            if (diff <= 60) {
                                sendto(socketfd, pkt->buf,  pkt->len, 0, (struct sockaddr *)&addr1, sizeof(struct sockaddr_in));
                            }
                        }
                    } 
...
static void *network_send_proc(void *arg)
{
    ...
    threadArg *threadarg = arg;
    struct ts_units *pkt;
    //printf("%d == %d\n", threadarg->fd, threadarg->ch);
    struct timeval tv1;
    struct timeval tv2;
    long long time2,time3;
    /*
    int socketfd;
    struct sockaddr_in addr1;
    ...
    */
    //pthread_t tid0;
	//pthread_create(&tid0, NULL, thread_function, (void*)threadarg);
            ...
            if (pkt)
            {
                if(stream_tag == 1){
                    ...
                    if (i == 0) {
                        ...
                        {
                            if (stream_key2 == 1) {
                                ps_file2 = fopen("/home/1test.264", "w+");
                                gettimeofday(&tv2, NULL);
                                time2 = tv2.tv_sec;
                            }
                        }
                        ...
                        {   
                            gettimeofday(&tv2, NULL);
                            time3 = tv2.tv_sec;
                            if((time3 - time2) <= recordTime)
                                fwrite(pkt->buf, sizeof(char), pkt->len, ps_file2);
                            if ((time3 - time2) == recordTime) {
                                stream_key2 = 2;
                            }
                        }
...
int libpes_init(void)
{
    ...
    int fd;
    int i;
    threadArg* threadArg0 = NULL;
    threadArg* threadArg1 = NULL;
...
#endif
        threadArg0 = malloc(sizeof(*threadArg0));
        threadArg1 = malloc(sizeof(*threadArg1));
        threadArg0->fd = fd;
        threadArg1->fd = fd;
        threadArg0->ch = 0;
        threadArg1->ch = 1;
        if (pthread_create(&thread0, &attr, network_send_proc, (void*)threadArg0) != 0)
        {
            printf("Error: Create libpes thread0 failed!\n");
            _VLC_UNLOCK();
            return -1;
        }
        /*
        if (pthread_create(&thread1, &attr, network_send_proc, (void*)threadArg1) != 0)
        {
            printf("Error: Create libpes thread1 failed!\n");
            _VLC_UNLOCK();
            return -1;
        }*/
